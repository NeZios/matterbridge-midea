...
import { MideaCloudClient, MideaLanDiscovery, MideaMode, type MideaAcState, type MideaCredentialCandidate, type MideaDeviceConfig, MideaLanAcDevice } from './midea-device.js';
...
export class MideaPlatform extends MatterbridgeDynamicPlatform {
  ...
  private async runUserCommand(device: MideaLanAcDevice, command: () => Promise<MideaAcState>): Promise<void> {
    try {
      ...
      if (registered) this.deferStateSync(registered, state);
    } catch (error) {
    ...
    try {
      for (const registeredAc of this.registeredAcs.values()) {
        ...
        try {
          ...
          if (registered) await this.syncRegisteredState(registered, state);
        } catch (error) {
  ...
  private async updateAuxiliaryEndpointState(
    ...
    try {
      ...
      if (!previous || previous.swingVertical !== state.swingVertical) await swingVerticalEndpoint.setAttribute(OnOff, 'onOff', state.swingVertical, this.log);

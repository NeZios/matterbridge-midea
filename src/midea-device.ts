...
export class MideaCloudClient {
  ...
  private readonly pushToken = randomBytes(90).toString('base64url');
  private apiUrl = DEFAULT_API_URL;
  private loginId = '';
  private uid = '';
  private headerAccessToken = '';
  private dataKey?: string;
  private dataIv?: string;
   ...
   */
  async getTokenKey(applianceId: string): Promise<{ token: string; key: string }> {
    const [first] = await this.getTokenKeyCandidates(applianceId);
    if (!first) throw new Error(`No token/key found for appliance ${applianceId}`);
   ...
   */
  async getTokenKeyCandidates(applianceId: string): Promise<MideaCredentialCandidate[]> {
    const udpIds = getUdpIds(Number(applianceId));
    const errors: string[] = [];
    const candidates: MideaCredentialCandidate[] = [];
    ...
    const unique = new Map<string, MideaCredentialCandidate>();
    for (const candidate of candidates) unique.set(`${candidate.token}:${candidate.key}`, candidate);
    if (unique.size > 0) return [...unique.values()];

    throw new Error(`No token/key found for appliance ${applianceId}. Attempts: ${errors.join(' | ')}`);
  }

  private async getTokenKeyByUdpId(udpId: string): Promise<MideaCredentialCandidate> {
  ...
  private async resolveRegion(): Promise<void> {
    try {
      ...
      if (typeof masUrl === 'string' && masUrl.startsWith('https://')) this.apiUrl = masUrl;
    } catch {
      this.apiUrl = DEFAULT_API_URL;
    }
  }

  private async getLoginId(): Promise<string> {
    const response = await this.apiRequest('/v1/user/login/id/get', { loginAccount: this.account }, false);
    const loginId = response.loginId;
    if (typeof loginId !== 'string' || loginId.length === 0) throw new Error('Midea cloud did not return a loginId');
    return loginId;
  }

  private async apiRequest(endpoint: string, data: ApiObject, authenticate = true, decoratePayload = true): Promise<ApiObject> {
    if (authenticate && !this.dataKey) await this.login();
    ...
    if (this.headerAccessToken) headers.accessToken = this.headerAccessToken;
  ...
  private setAccessToken(token: string, randomData: string): void {
    if (!token || !randomData) throw new Error('Midea cloud login did not return encryption material');
  ...
  private aesDecryptString(value: string, key = this.dataKey, iv = this.dataIv): string {
    if (!key) throw new Error('Midea cloud data key is not initialized');
...
class NetHomePlusCloudClient {
  private readonly appId = '1017';
  private readonly loginKey = '3742e9e5842d4ad59c2db887e12449f9';
  private readonly apiUrl = 'https://mapp.appsmb.com';
  private readonly deviceId = randomBytes(8).toString('hex');
  private sessionId = '';
  private accessToken = '';
  private uid = '';
  ...
  async login(): Promise<void> {
    ...
    if (!this.sessionId) throw new Error('NetHome Plus login did not return a sessionId');
  }

  async getTokenKeyByUdpId(udpId: string): Promise<MideaCredentialCandidate> {
    const response = await this.apiRequest('/v1/iot/secure/getToken', { udpid: udpId });
  ...
  private async getLoginId(): Promise<string> {
    const response = await this.apiRequest('/v1/user/login/id/get', { loginAccount: this.account }, false);
    const loginId = response.loginId;
    if (typeof loginId !== 'string' || loginId.length === 0) throw new Error('NetHome Plus did not return a loginId');
    return loginId;
  }

  private async apiRequest(endpoint: string, data: ApiObject, authenticate = true): Promise<ApiObject> {
    if (authenticate && !this.sessionId) await this.login();
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        ...
        if (code === '0') return asRecord(responsePayload.result ?? responsePayload);
  ...
  private sign(url: string, payload: Record<string, string>): string {
  ...
  private encryptPassword(loginId: string, password: string): string {
...
export class MideaLanDiscovery {
   ...
   */
  async discover(timeoutMs = 8000): Promise<DiscoveredLanDevice[]> {
    const socket = dgram.createSocket('udp4');
    const found = new Map<string, DiscoveredLanDevice>();
    ...
    socket.on('message', (message, info) => {
      try {
        ...
        if (normalizeApplianceType(device.type) === AC_TYPE) found.set(device.id, device);
      } catch {
    ...
    for (const address of broadcastAddresses()) {
      for (const port of DISCOVERY_PORTS) socket.send(DISCOVERY_MESSAGE, port, address);
    }
...
export class MideaLanAcDevice {
  ...
  private lastManualFanSpeed = 80;
  private state: MideaAcState = {
    power: false,
    mode: MideaMode.Auto,
  ...
  private readonly localSecurity: LocalSecurity;
   ...
   */
  constructor(private readonly config: MideaDeviceConfig) {
   ...
   */
  async refresh(): Promise<MideaAcState> {
   ...
   */
  async setPower(power: boolean): Promise<MideaAcState> {
   ...
   * @returns {Promise<MideaAcState>} AC state after the command.
   */
  async setMode(mode: MideaMode, power = true): Promise<MideaAcState> {
   ...
   * @returns {Promise<MideaAcState>} AC state after the command.
   */
  async setTargetTemperature(targetTemperature: number): Promise<MideaAcState> {
   ...
   * @returns {Promise<MideaAcState>} AC state after the command.
   */
  async setFanSpeed(fanSpeed: number): Promise<MideaAcState> {
    if (fanSpeed === 0) return this.setPower(false);
   ...
   * @returns {Promise<MideaAcState>} AC state after the command.
   */
  async setFanAuto(auto: boolean): Promise<MideaAcState> {
    if (auto) this.rememberManualFanSpeed(this.state.fanSpeed);
   ...
   * @returns {Promise<MideaAcState>} AC state after the command.
   */
  async setSwingVertical(swingVertical: boolean): Promise<MideaAcState> {
   ...
   * @returns {Promise<MideaAcState>} AC state after the command.
   */
  async setEcoMode(ecoMode: boolean): Promise<MideaAcState> {
  ...
  private async apply(update: Partial<MideaAcState>): Promise<MideaAcState> {
    const next = {
      ...this.state,
      ...update,
  ...
  private rememberManualFanSpeed(fanSpeed: number | undefined): void {
    if (fanSpeed !== undefined && fanSpeed > 0 && fanSpeed !== AUTO_FAN_SPEED) {
      this.lastManualFanSpeed = fanSpeed;
    }
  }

  private async sendLocalCommand(command: Buffer): Promise<Buffer> {
    return this.withCommandLock(async () => this.sendLocalCommandUnlocked(command));
  }

  private async withCommandLock<T>(task: () => Promise<T>): Promise<T> {
  ...
  private async sendLocalCommandUnlocked(command: Buffer): Promise<Buffer> {
    this.localSecurity.reset();
    const socket = await connectTcp(this.config.ip, this.config.port);
    try {
...
class LocalSecurity {
  private tcpKey: Buffer<ArrayBufferLike> = Buffer.alloc(0);
  ...
  md5Signature(data: Buffer): Buffer {
    return createHash('md5')
      .update(Buffer.concat([data, SIGN_SALT]))
      .digest();
  }

  setTcpKeyFromResponse(response: Buffer): void {
    if (response.toString() === 'ERROR') throw new Error('Midea v3 authentication returned ERROR');
    if (response.length !== 64) throw new Error(`Invalid Midea v3 authentication response length: ${response.length}`);
    const payload = response.subarray(0, 32);
    const sign = response.subarray(32);
    const plain = aesCbcDecrypt(payload, this.key);
    if (!createHash('sha256').update(plain).digest().equals(sign)) throw new Error('Midea v3 authentication signature mismatch');
    this.tcpKey = xorBuffers(plain, this.key);
    this.requestCount = 0;
  }

  encode8370(dataToEncrypt: Buffer, messageType: number): Buffer {
    ...
    if (messageType === 0x06 || messageType === 0x03) {
      if (this.tcpKey.length === 0) throw new Error('Midea v3 tcp key is not initialized');
  ...
  decode8370(input: Buffer): Buffer[] {
    const messages: Buffer[] = [];
    let data = input;
    while (data.length >= 6) {
      const header = data.subarray(0, 6);
      if (header[0] !== 0x83 || header[1] !== 0x70) throw new Error('Midea response is not a v3 8370 packet');
      ...
      if (messageType === 0x06 || messageType === 0x03) {
        ...
        if (padding > 0) packet = packet.subarray(0, packet.length - padding);
      }
...
function buildLanPacket(deviceId: number, command: Buffer): Buffer {
...
function decryptLanPacket(message: Buffer): Buffer {
  if (message.length <= 56) throw new Error('Midea LAN packet is too short');
...
function extractAcPayload(decrypted: Buffer): Buffer | undefined {
  const index = decrypted.indexOf(0xc0);
  if (index === -1) return undefined;
  return decrypted.subarray(index);
}

function parseDiscoveryMessage(message: Buffer, ip: string): DiscoveredLanDevice {
  let version: 2 | 3;
  let buffer = message;
...
function readMideaFrame(socket: net.Socket, timeoutMs: number): Promise<Buffer> {
  return new Promise((resolve, reject) => {
...
function expectedMideaFrameLength(buffer: Buffer): number | undefined {
...
function isMideaErrorResponse(buffer: Buffer): boolean {
  if (buffer.toString() === 'ERROR') return true;
  if (buffer.length >= 13 && buffer[0] === 0x83 && buffer[1] === 0x70 && buffer.subarray(8).toString() === 'ERROR') return true;
  return false;
}

function splitV2Messages(input: Buffer): Buffer[] {
  const messages: Buffer[] = [];
  let data = input;
  while (data.length >= 6) {
...
function numberToBuffer(value: number, length: number, endian: 'little' | 'big'): Buffer {
...
function getUdpIds(applianceId: number): string[] {
  if (!Number.isFinite(applianceId) || applianceId <= 0) return [];
  return [...new Set([getUdpId(applianceId, 'little'), getUdpId(applianceId, 'big')])];
}

function getUdpId(applianceId: number, endian: 'little' | 'big'): string {
  const digest = createHash('sha256')
    .update(numberToBuffer(applianceId, 6, endian))
    .digest();
  const output = Buffer.alloc(16);
  for (let index = 0; index < 16; index += 1) output[index] = digest[index] ^ digest[index + 16];
  return output.toString('hex');
}

function parseTokenKeyResponse(response: ApiObject, udpId: string): { token: string; key: string } {
  ...
  if (!isHexCredential(token) || !isHexCredential(key)) throw new Error('token/key not returned for this udpId');

  return { token, key };
}

function stringifyRecord(data: ApiObject): StringRecord {
...
function aesCbcDecrypt(data: Buffer, key: Buffer): Buffer {
...
function xorBuffers(left: Buffer, right: Buffer): Buffer {
  ...
  for (let index = 0; index < left.length; index += 1) output[index] = left[index] ^ right[index % right.length];
  return output;
}

function buildStatusCommand(): Buffer {
  return finalizeSequenceCommand(
    Buffer.from([
...
function buildSetCommand(state: MideaAcState): Buffer {
  ...
  if (state.power) data[11] |= 0b00000001;
  data[11] |= 0b01000000;
...
function finalizeSequenceCommand(data: Buffer): Buffer {
 ...
 */
export function parseAcState(payload: Buffer, previous: MideaAcState): MideaAcState {
  const payloadStart = payload[0] === 0xc0 ? 0 : payload.indexOf(0xc0);
  if (payloadStart < 0) return previous;

  const data = payload.subarray(payloadStart);
  if (data.length < 23 || data[0] !== 0xc0) return previous;
...
function parseMideaMode(value: number): MideaMode {
 ...
 */
export function parseTemperature(raw: number, decimal: number): number | undefined {
  if (!Number.isFinite(raw) || !Number.isFinite(decimal)) return undefined;
  if (raw === 0 || raw === 0xff) return undefined;
  const base = (raw - 50) / 2;
...
function signProxied(data: string, random: string): string {
  return createHmac('sha256', HMAC_KEY).update(`${IOT_KEY}${data}${random}`).digest('hex');
}

function encryptPassword(loginId: string, password: string): string {
  const first = createHash('sha256').update(password, 'ascii').digest('hex');
  return createHash('sha256').update(`${loginId}${first}${APP_KEY}`, 'ascii').digest('hex');
}

function encryptIamPassword(loginId: string, password: string): string {
...
function normalizeApplianceType(value: unknown): string {
  if (typeof value === 'number') return `0x${value.toString(16)}`;
  const text = String(value ?? '').toLowerCase();
  if (text === 'ac') return AC_TYPE;
  return text.startsWith('0x') ? text : `0x${text}`;
}

function asRecord(value: unknown): ApiObject {
  return value !== null && typeof value === 'object' ? (value as ApiObject) : {};
}

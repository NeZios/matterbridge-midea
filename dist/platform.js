...
export class MideaPlatform extends MatterbridgeDynamicPlatform {
    cloud;
    pollTimer;
    pendingCloudBootstrap;
    stateSyncTimers = new Set();
    registeredAcs = new Map();
    updatingFromCloud = false;
    applyingConfigChange = false;
    ...
    async onShutdown(reason) {
        ...
        for (const timer of this.stateSyncTimers)
            clearTimeout(timer);
    ...
    scheduleReload(reason, saveAfterBootstrap, allowCloudBootstrap, delayMs) {
        if (this.pendingCloudBootstrap)
            clearTimeout(this.pendingCloudBootstrap);
        this.log.info(`Scheduling Midea reload after ${reason}`);
        this.pendingCloudBootstrap = setTimeout(() => {
            this.pendingCloudBootstrap = undefined;
            void this.reloadDevices(saveAfterBootstrap, allowCloudBootstrap).catch((error) => {
                this.log.error(`Midea reload failed: ${error instanceof Error ? error.message : String(error)}`);
            });
        }, delayMs);
    }
    async reloadDevices(saveAfterBootstrap = false, allowCloudBootstrap = true) {
        if (this.stopped)
            return;
            ...
            try {
                ...
                if (this.validateDevice([deviceConfig.id, deviceDisplayName(deviceConfig), deviceConfig.sn])) {
                    ...
                    if (initialState) {
                        ...
                        if (registered)
                            await this.syncRegisteredState(registered, initialState);
                    }
                }
            }
            catch (error) {
    ...
    async bootstrapDevices(allowCloudBootstrap = true) {
        ...
        const password = typeof this.config.password === 'string' ? this.config.password : '';
        const hasCloudCredentials = Boolean(username && password);
        const needsBootstrap = configured.length === 0 ||
        ...
        if (!allowCloudBootstrap) {
            if (hasCloudCredentials)
                this.log.info('Midea cloud bootstrap deferred until after plugin startup');
            return;
        }
        if (!username || !password) {
        ...
        for (const lan of lanDevices) {
            if (normalizeType(lan.type) !== '0xac')
                continue;
            if (bootstrapTargets.some((appliance) => appliance.id === lan.id || appliance.sn === lan.sn))
                continue;
        ...
        for (const configuredDevice of configured) {
            if (normalizeType(configuredDevice.type) !== '0xac')
                continue;
            if (bootstrapTargets.some((appliance) => appliance.id === configuredDevice.id || appliance.sn === configuredDevice.sn))
                continue;
    ...
    async selectWorkingCredentials(device, candidates) {
        if (candidates.length === 0)
            throw new Error('No token/key candidates returned');
        const errors = [];
    ...
    async createEndpoint(appliance, state) {
        ...
        const endpoint = new MatterbridgeEndpoint([roomAirConditioner, powerSource], { id: sanitizeEndpointId(appliance.id) })
            ...
            .createDefaultThermostatUserInterfaceConfigurationClusterServer(ThermostatUserInterfaceConfiguration.TemperatureDisplayMode.Celsius, ThermostatUserInterfaceConfiguration.KeypadLockout.NoLockout, ThermostatUserInterfaceConfiguration.ScheduleProgrammingVisibility.ScheduleProgrammingPermitted)
    ...
    async createFanEndpoint(appliance, state) {
        const fanMode = state ? mideaFanSpeedToMatter(state.power, state.fanSpeed) : FanControl.FanMode.Auto;
        ...
        return new MatterbridgeEndpoint([fan, powerSource], { id: `${sanitizeEndpointId(appliance.id)}-fan` })
            .createDefaultBridgedDeviceBasicInformationClusterServer(`${name} Fan`, `${appliance.sn}-fan`, this.matterbridge.aggregatorVendorId, MANUFACTURER, `${PRODUCT} Fan`, 10000, PLUGIN_VERSION)
            .createDefaultIdentifyClusterServer()
            .createDefaultPowerSourceWiredClusterServer()
            .createDeadFrontOnOffClusterServer(state?.power ?? false)
    ...
    async createSwitchEndpoint(appliance, label, idSuffix, initialValue) {
        const name = deviceDisplayName(appliance);
        return new MatterbridgeEndpoint([onOffPlugInUnit, powerSource], { id: `${sanitizeEndpointId(appliance.id)}-${idSuffix}` })
            .createDefaultBridgedDeviceBasicInformationClusterServer(`${name} ${label}`, `${appliance.sn}-${idSuffix}`, this.matterbridge.aggregatorVendorId, MANUFACTURER, `${PRODUCT} ${label}`, 10000, PLUGIN_VERSION)
            .createDefaultIdentifyClusterServer()
            .createDefaultPowerSourceWiredClusterServer()
            .createDeadFrontOnOffClusterServer(initialValue)
            .addRequiredClusters();
    }
    bindEndpoint(endpoint, device) {
        endpoint
            ...
            .subscribeAttribute(OnOff, 'onOff', async (value, _oldValue, context) => {
            if (this.shouldIgnoreAttributeWrite(context))
                return;
            await this.runUserCommand(device, () => device.setPower(value));
        })
            .subscribeAttribute(Thermostat, 'systemMode', async (value, _oldValue, context) => {
            if (this.shouldIgnoreAttributeWrite(context))
                return;
            await this.runUserCommand(device, () => device.setMode(matterModeToMidea(value), value !== Thermostat.SystemMode.Off));
        })
            .subscribeAttribute(Thermostat, 'occupiedCoolingSetpoint', async (value, _oldValue, context) => {
            if (this.shouldIgnoreAttributeWrite(context))
                return;
            await this.runUserCommand(device, () => device.setTargetTemperature(matterTemperatureToCelsius(value)));
        })
            .subscribeAttribute(Thermostat, 'occupiedHeatingSetpoint', async (value, _oldValue, context) => {
            if (this.shouldIgnoreAttributeWrite(context))
                return;
            await this.runUserCommand(device, () => device.setTargetTemperature(matterTemperatureToCelsius(value)));
        })
            .subscribeAttribute(FanControl, 'fanMode', async (value, _oldValue, context) => {
            if (this.shouldIgnoreAttributeWrite(context))
                return;
            await this.runUserCommand(device, () => device.setFanSpeed(matterFanModeToMidea(value)));
        })
            .subscribeAttribute(FanControl, 'percentSetting', async (value, _oldValue, context) => {
            if (this.shouldIgnoreAttributeWrite(context))
                return;
            await this.runUserCommand(device, () => device.setFanSpeed(matterFanPercentToMidea(value)));
        });
    }
    bindFanEndpoint(endpoint, device) {
        endpoint
            .addCommandHandler('on', async () => {
            await this.runUserCommand(device, () => device.setPower(true));
        })
    ...
    bindSwitchEndpoint(endpoint, device, setter) {
        endpoint
            .addCommandHandler('on', async () => {
    ...
    shouldIgnoreAttributeWrite(_context) {
        return this.updatingFromCloud;
    }
    async runUserCommand(device, command) {
        try {
    ...
    deferStateSync(registered, state) {
        if (this.stopped)
            return;
        const timer = setTimeout(() => {
            this.stateSyncTimers.delete(timer);
            if (this.stopped || this.registeredAcs.get(registered.config.id) !== registered)
                return;
            void this.syncRegisteredState(registered, state).catch((error) => {
    ...
    async syncRegisteredState(registered, state) {
        ...
        if (previous && sameAcState(previous, state))
            return;
    ...
    async updateFanEndpointState(endpoint, state, previous) {
        if (previous && previous.power === state.power && previous.fanSpeed === state.fanSpeed)
            return;
        this.updatingFromCloud = true;
        try {
    ...
    async updateAuxiliaryEndpointState(fanAutoEndpoint, swingVerticalEndpoint, ecoEndpoint, state, previous) {
        this.updatingFromCloud = true;
        try {
 ...
 */
export function parseMideaDeviceConfigs(value) {
 ...
 */
export function isValidMideaDeviceConfig(value) {
...
function sanitizeEndpointId(value) {
...
function sameAcState(left, right) {
...
function isRealConfiguredDevice(device) {
...
function isHexCredential(value) {
 ...
 */
export function celsiusToMatterTemperature(value) {
 ...
 */
export function celsiusToMatterSetpoint(value) {
    if (!Number.isFinite(value))
        return MIN_SETPOINT * 100;
    return Math.round(value) * 100;
}
/**
 ...
 */
export function matterTemperatureToCelsius(value) {
    return clamp(Math.round(value / 100), MIN_SETPOINT, MAX_SETPOINT);
}
function thermostatSetpointsForState(state) {
    const target = Math.round(state?.targetTemperature ?? 24);
    if (!state?.power)
        return { heating: MIN_SETPOINT, cooling: target };
    if (state.mode === MideaMode.Heat)
        return { heating: target, cooling: MAX_SETPOINT };
    return { heating: MIN_SETPOINT, cooling: target };
}
...
function mideaModeToMatter(power, mode) {
    if (!power)
        return Thermostat.SystemMode.Off;
    if (mode === MideaMode.Cool)
        return Thermostat.SystemMode.Cool;
    if (mode === MideaMode.Heat)
        return Thermostat.SystemMode.Heat;
    if (mode === MideaMode.FanOnly)
        return thermostatSystemMode('FanOnly', 7);
    if (mode === MideaMode.Dry)
        return thermostatSystemMode('Dry', 8);
    return Thermostat.SystemMode.Auto;
}
function matterModeToMidea(mode) {
    if (mode === Thermostat.SystemMode.Cool)
        return MideaMode.Cool;
    if (mode === Thermostat.SystemMode.Heat)
        return MideaMode.Heat;
    if (mode === thermostatSystemMode('FanOnly', 7))
        return MideaMode.FanOnly;
    if (mode === thermostatSystemMode('Dry', 8))
        return MideaMode.Dry;
    return MideaMode.Auto;
}
function thermostatSystemMode(name, fallback) {
    const value = Reflect.get(Thermostat.SystemMode, name);
    return typeof value === 'number' ? value : fallback;
}
function mideaFanSpeedToMatter(power, speed) {
    if (!power)
        return FanControl.FanMode.Off;
    if (speed === undefined || speed === 102)
...
function matterFanModeToMidea(mode) {
    if (mode === FanControl.FanMode.Off)
        return 0;
    if (mode === FanControl.FanMode.Low)
        return 33;
    if (mode === FanControl.FanMode.Medium)
        return 66;
    if (mode === FanControl.FanMode.High)
        return 100;
    return 102;
}
/**
 ...
 */
export function matterFanPercentToMidea(value) {
 ...
 */
export function mideaFanSpeedToPercent(speed) {
    if (speed !== undefined && !Number.isFinite(speed))
        return 0;
    if (speed === undefined || speed <= 0 || speed === 102)
        return 0;
